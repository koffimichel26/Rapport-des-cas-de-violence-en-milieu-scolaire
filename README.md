...
// ══════════════════════════════════════════════
//  GÉNÉRATION GRAPHIQUES HORS-ÉCRAN pour PDF
//  Crée des canvas indépendants (jamais display:none)
// ══════════════════════════════════════════════
function buildOffscreenChart(type, data, options, w=900, h=420) {
  return new Promise(resolve => {
    const canvas = document.createElement('canvas');
    canvas.width  = w;
    canvas.height = h;
    // IMPORTANT : le canvas doit être dans le DOM (position absolute hors vue)
    canvas.style.cssText = 'position:fixed;left:-9999px;top:-9999px;width:'+w+'px;height:'+h+'px;';
    document.body.appendChild(canvas);

    const baseOpts = {
      responsive: false,
      animation: { duration: 0 },   // pas d'animation = rendu immédiat
      plugins: {
    ...
    // Fusion des options
    const mergedOpts = JSON.parse(JSON.stringify(baseOpts));
    if (options) {
      if (options.plugins) Object.assign(mergedOpts.plugins, options.plugins);
      if (options.scales)  Object.assign(mergedOpts.scales,  options.scales);
      if (options.indexAxis) mergedOpts.indexAxis = options.indexAxis;
    }

    const chart = new Chart(canvas, { type, data, options: mergedOpts });

    // Attendre le prochain frame pour que Chart.js ait dessiné
    requestAnimationFrame(() => {
      requestAnimationFrame(() => {
        // Fond blanc + copie dans un canvas propre
...
async function generatePDF() {
  ...
  try {
    // ── 1. Générer toutes les images de graphiques hors-écran ──
    const chartImages = await renderAllChartsForPDF();
    ...
    function checkPage(extra=0) { if (y + extra > H-18) newPage(); }
    ...
    function insertChart(imgDataUrl, label, fullWidth=true, hMM=78) {
      const wMM = fullWidth ? contentW : (contentW - 5) / 2;
      checkPage(hMM + 14);
      ...
      doc.rect(margin, y, wMM, hMM);
      doc.addImage(imgDataUrl, 'PNG', margin+1, y+1, wMM-2, hMM-2);
      y += hMM + 7;
    }

    function insertChartPair(left, right, hMM=78) {
    ...
    doc.text(school, W/2, 23, { align:'center' });
    doc.setFontSize(9);
    doc.text(`Année scolaire : ${year}   |   Généré le ${new Date().toLocaleDateString('fr-FR')}`, W/2, 32, { align:'center' });
    doc.setTextColor(0,0,0);
    y = 52;

    // ══════════ RÉSUMÉ ══════════
    sectionTitle('RÉSUMÉ GLOBAL');
    addText(`Total d'incidents enregistrés sur l'année : ${gt}`, 11, true);
    addText(`1er Trimestre : ${t1}   |   2e Trimestre : ${t2}   |   3e Trimestre : ${t3}`, 10);
    y += 5;

    // ══════════ TABLEAU DES INCIDENTS ══════════
    sectionTitle('TABLEAU DES INCIDENTS');
    const hdr = ['Trim.','Niveau','Type de violence','Lieu','Acteurs','Nb'];
    const cW  = [18,16,54,30,32,14];
    let x = margin;
    doc.setFillColor(10,22,40); doc.rect(margin,y,contentW,7,'F');
    doc.setTextColor(255,255,255); doc.setFont('helvetica','bold'); doc.setFontSize(8);
    hdr.forEach((h,i)=>{ doc.text(h,x+2,y+5); x+=cW[i]; });
    ...
    incidents.forEach((inc,idx)=>{
      ...
      [inc.trimestre,inc.niveau,inc.type,inc.lieu,inc.acteurs,''+inc.nb].forEach((cell,i)=>{
    ...
    const sH=['Niveau','T1','T2','T3','Total','%'], sC=[28,22,22,22,26,22];
    x=margin;
    doc.setFillColor(10,22,40); doc.rect(margin,y,contentW,7,'F');
    doc.setTextColor(255,255,255); doc.setFont('helvetica','bold'); doc.setFontSize(8);
    sH.forEach((h,i)=>{ doc.text(h,x+2,y+5); x+=sC[i]; });
    ...
    NIVEAUX.forEach((n,idx)=>{
      ...
      const nt2=incidents.filter(i=>i.trimestre==='T2'&&i.niveau===n).reduce((s,i)=>s+i.nb,0);
      const nt3=incidents.filter(i=>i.trimestre==='T3'&&i.niveau===n).reduce((s,i)=>s+i.nb,0);
      const tot=nt1+nt2+nt3, pct=gt?((tot/gt)*100).toFixed(1):'0.0';
      if(idx%2===0){doc.setFillColor(245,248,252);doc.rect(margin,y,contentW,6,'F');}
      x=margin; doc.setFontSize(8); doc.setFont('helvetica','normal');
      [''+n,''+nt1,''+nt2,''+nt3,''+tot,pct+'%'].forEach((v,i)=>{ doc.text(v,x+2,y+4); x+=sC[i]; });
      y+=6;
    });
    checkPage(7);
    doc.setFillColor(10,22,40); doc.rect(margin,y,contentW,7,'F');
    doc.setTextColor(255,255,255); doc.setFont('helvetica','bold'); doc.setFontSize(8);
    x=margin;
    ['TOTAL',''+t1,''+t2,''+t3,''+gt,'100%'].forEach((v,i)=>{ doc.text(v,x+2,y+5); x+=sC[i]; });
    doc.setTextColor(0,0,0); y+=11;

    // ══════════ ANALYSE AUTOMATIQUE ══════════
    sectionTitle('ANALYSE AUTOMATIQUE');
    if(t2<t1&&t1>0) addText(`• Baisse observée au 2e trimestre : ${t1} → ${t2} incidents (−${t1-t2})`,10);
    ...
    const topN2=NIVEAUX.map(n=>({n,tot:getTotalByNiveau(n)})).sort((a,b)=>b.tot-a.tot)[0];
    if(topN2.tot>0) addText(`• Niveau le plus touché : ${topN2.n} (${topN2.tot} incidents)`,10);
    const topT2=TYPES.map(t=>({t,tot:getTotalByType(t)})).sort((a,b)=>b.tot-a.tot)[0];
    if(topT2.tot>0) addText(`• Type dominant : ${topT2.t} (${topT2.tot} cas)`,10);
    const topL2=LIEUX.map(l=>({l,tot:getTotalByLieu(l)})).sort((a,b)=>b.tot-a.tot)[0];
    if(topL2.tot>0) addText(`• Lieu le plus concerné : ${topL2.l} (${topL2.tot} incidents)`,10);
    y+=5;

    // ══════════ PAGE GRAPHIQUES 1 ══════════
    newPage();
    doc.setFillColor(10,22,40); doc.rect(0,0,W,14,'F');
    doc.setTextColor(255,255,255); doc.setFont('helvetica','bold'); doc.setFontSize(13);
    doc.text('VISUALISATIONS STATISTIQUES', W/2, 9, {align:'center'});
    doc.setTextColor(0,0,0); y=20;

    // Ligne 1 : Trimestres (gauche) + Camembert (droite)
    insertChartPair(
      { label: chartImages[0].label, img: chartImages[0].img },
      { label: chartImages[2].label, img: chartImages[2].img },
      82
    );

    // Types par trimestre (pleine largeur)
    insertChart(chartImages[1].img, chartImages[1].label, true, 88);
    ...
    // Lieux et acteurs (pleine largeur)
    insertChart(chartImages[3].img, chartImages[3].label, true, 82);

    // Ligne : par niveau (gauche) + comparaison (droite)
    insertChartPair(
      { label: chartImages[4].label, img: chartImages[4].img },
      { label: chartImages[5].label, img: chartImages[5].img },
      88
    );

    // ══════════ COMMENTAIRES ══════════
    const hasContent = comments.global
      || NIVEAUX.some(n=>comments.niveaux&&comments.niveaux[n])
      || comments.conclusion
      || [1,2,3].some(q=>{const m=meta['T'+q];return m&&(m.msg||m.mesures);});
    ...
    if (hasContent) {
      ...
      if (comments.global) {
        ...
        addText(comments.global, 10); y+=3;
      }
      ...
      if (hasNivCom) {
        ...
        NIVEAUX.forEach(n=>{
          if(comments.niveaux&&comments.niveaux[n]){
            checkPage(14); addText(n+' :',10,true); addText(comments.niveaux[n],10,false,5); y+=2;
          }
        });
      }
    ...
    for (let i=1; i<=pageCount; i++) {
      ...
      doc.text(new Date().toLocaleDateString('fr-FR'), W-margin, H-4, {align:'right'});
    }
